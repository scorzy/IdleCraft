...
describe('createEnemies', () => {
    ...
    it('creates the requested quantity of enemies', () => {
        ...
        enemyIds.forEach((id) => {
            ...
            expect(state.characters.entries[id]!.templateId).toBe(CharTemplateEnum.Wolf)
        })
    })

    it('creates enemies for multiple templates', () => {
        createEnemies(state, [
            { quantity: 1, template: CharTemplateEnum.Wolf },
            { quantity: 2, template: CharTemplateEnum.Chicken },
        ])
        ...
        expect(templateCounts[CharTemplateEnum.Wolf]).toBe(1)
        expect(templateCounts[CharTemplateEnum.Chicken]).toBe(2)
    })

    it('resets health, mana and stamina to their max for created enemies', () => {
        createEnemies(state, [{ quantity: 1, template: CharTemplateEnum.Boar }])
