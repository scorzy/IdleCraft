...
const ComboBoxResponsiveContext =
  React.createContext<ComboBoxResponsiveContextValue | null>(null)
    ...
    <>
      {filter && (
        <ComboboxInput
          ...
          className="m-1 mb-0 h-8 border-input/30 bg-input/30 shadow-none"
        />
      )}
      {showEmpty && (
...
function ComboBoxItem({
  ...
  const selected =
    selectedProp ?? (context?.selectedId !== null && context?.selectedId === value)
  const searchText = React.useMemo(
    () =>
      [value, getTextContent(children), getTextContent(bottomSlot)]
        .filter(Boolean)
        .join(" ")
        .toLocaleLowerCase(),
  ...
  const matches =
    !context?.query ||
    searchText.includes(context.query.trim().toLocaleLowerCase())
  ...
  const handleClick = React.useCallback<
    NonNullable<ComboboxPrimitive.Item.Props["onClick"]>
  >(
    (event) => {
      onClick?.(event)
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {children}
        {bottomSlot && (
          <div className="mt-2 text-sm leading-none text-muted-foreground">
            {bottomSlot}
          </div>
        )}
      </div>
      <span className="ml-auto flex shrink-0 items-center gap-2">
        {rightSlot}
        <CheckIcon
          className={cn("text-muted-foreground", { invisible: !selected })}
        />
      </span>
    </ComboboxPrimitive.Item>
  )
...
function ComboBoxTrigger({
  ...
  ComboBoxItemProps,
  "children" | "className" | "icon" | "rightSlot" | "bottomSlot" | "selected"
> & {
  value?: string
}) {
  const context = React.useContext(ComboBoxResponsiveContext)
  const selected =
    selectedProp ?? (value != null && context?.selectedId === value)
