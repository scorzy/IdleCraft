...
export const selectExp = (expType: ExpEnum) => (state: GameState) =>
    CharacterAdapter.selectEx(state.characters, PLAYER_ID).skillsExp[expType] ?? 0

export const selectLevelExp = (expType: ExpEnum) => (state: GameState) => {
    const level = CharacterAdapter.selectEx(state.characters, PLAYER_ID).skillsLevel[expType] ?? 0
