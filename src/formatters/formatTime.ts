...
export function getTimeFormatterPrecise(numberFormat: (num: number) => string, t: MsgFunctions) {
    return (time: number) => formatTimePrecise(time, numberFormat, t)
}
...
export function formatTimePrecise(time: number, numberFormat: (num: number) => string, t: MsgFunctions): string {
    time /= 1000
    ...
    if (days > 0) return `${t.days(days, numberFormat(days))} ${t.h(hours, numberFormat(hours))}`

    const minutes = Math.floor((time / 60) % 60)

    if (hours > 0) return `${t.h(hours, numberFormat(hours))} ${t.m(minutes, numberFormat(minutes))}`

    const seconds = time % 60

    if (minutes > 0)
        return `${t.m(minutes, numberFormat(minutes))} ${t.s(Math.floor(seconds), Math.floor(seconds).toString())}`
