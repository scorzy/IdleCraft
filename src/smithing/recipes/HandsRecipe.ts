...
import { ARMS_ARMOUR_RATIO, BASE_ARMOUR } from '../../const'
...
const handsParams: RecipeParameterItemFilter[] = [
    {
        id: 'bar',
        nameId: 'Bar',
...
export const handsRecipe = makeMemoizedRecipe({
    id: 'HandsRecipe',
    nameId: 'Gauntlets',
    iconId: Icons.Gauntlet,
    type: RecipeTypes.Smithing,
    recipeGroup: RecipeGroups.Armours,
    getParameters: () => handsParams,
    getResult(state: GameState, params: RecipeParameterValue[]): RecipeResult | undefined {
        ...
        Object.entries(barArmourData).forEach(
            (kv) => (armourData[kv[0] as DamageTypes] = Math.floor(BASE_ARMOUR * ARMS_ARMOUR_RATIO * kv[1]))
        )
